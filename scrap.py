...
def scrape_posts(playwright):
    ...
    for topic in all_topics:
        ...
        if DATE_FROM <= created_at <= DATE_TO:
            ...
            # Add required metadata at top level for compatibility
            topic_data["id"] = topic["id"]
            topic_data["title"] = topic.get("title", "")
            topic_data["slug"] = topic.get("slug", "")

            filename = f"topic_{topic['id']}.json"
    ...
    print(f"✅ Saved {topic_count} topic files to {DISCOURSE_DIR}")
    browser.close()
